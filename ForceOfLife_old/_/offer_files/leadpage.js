;
// This is an Empty Var So User Javascript Knows When it Can Execute
var TEMPLATE_CONTEXT = 'published';
...
(function(window, query, App, undefined) {
  ...
  (function() {
    ...
    this.getLeadBoxId = function(href) {
      // grab the last "path" piece, ignoring ?uri=parms and trailing slash.
    ...
    this.getServerLocation = function(useServingDomain) {
      var host;
      var protocol = window.location.protocol;
      useServingDomain = false;  // TODO: remove this after foundry migration
      var meta = useServingDomain ? 'leadpages-serving-domain' : 'leadpages-url';
    ...
    this.createServerAddress = function(pathname, useServingDomain) {
      var server = this.getServerLocation(useServingDomain);
    ...
    this.copyUriParms = function(src_uri, dest_uri) {
      // This support may be pointless; does the page builder even LET THEM edit the URLs of the links to Leadboxes?
      // This might be of use to nobody except customers who export the HTML and then hand-edit their links.
      // Or possibly pages that import other JS that dynamically changes href attributes or something.
      if (src_uri.indexOf("?") < 0)
        return dest_uri;  // No parms to copy
      dest_uri += dest_uri.indexOf("?") !== -1 ? "&" : "?";
      dest_uri += src_uri.substr(src_uri.indexOf("?") + 1);
      return dest_uri;
    }
    ...
    this.setupExitMessage = function() {
      ...
      var location = this.createServerAddress(pathname, true);
    ...
    this.onClickOptin = function(leadbox_id) {
      if (mobile) {
        ...
        //TODO: Hmm, should we be adding the current window's URI parms here for consistency?
    ...
    this.showLeadBox = function(leadbox_id) {
      if (leadbox_id) {
        ...
        this.leadboxes[leadbox_id][0].contentWindow.postMessage({handler: "lpdisplay"}, '*');
      } else {
    ...
    this.setupLeadBoxes = function() {
      var shown = false;
      var location = this.createLeadBoxUrl(); // unused???
      var winwin = $(window);
          ...
          if (!href.match(/^https?:\/\//)) {
            ...
            if (optin && optin.length > 1 && !!optin[1]) {
              ...
              if (!self.leadboxes[optin[1]]) {
                ...
                location = self.copyUriParms(href, location);
                var lb = $('<iframe />')
                  .addClass('leadbox')
                  .attr('frameborder', 0)
                  .attr('src', location)
